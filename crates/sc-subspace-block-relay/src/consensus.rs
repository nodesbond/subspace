    ...
    ProtoServer: ProtocolServer<BlockHash<Block>> + Send,
{
    ...
    async fn on_request(&mut self, request: IncomingRequest) {
        ...
        if let Ok(msg) =
            <InitialRequest<Block, ProtoServer::Request> as Decode>::decode(&mut payload.as_ref())
        {
            server_msg = Some(ServerMessage::InitialRequest(msg));
        } else if let Ok(msg) = <ProtoServer::Request as Decode>::decode(&mut payload.as_ref()) {
