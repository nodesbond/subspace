    ...
    CBlock: BlockT,
{
    let true_invalid_bundle_entry = bad_receipt
        ...
        .ok_or(sp_domains::verification::VerificationError::BundleNotFound)?;

    if !true_invalid_fraud_proof.matches_with_bundle_entry(true_invalid_bundle_entry) {
        return Err(sp_domains::verification::VerificationError::FraudProofMismatch);
    }
    ...
    match true_invalid_fraud_proof.proof_data {
        ProofDataPerExpectedInvalidBundle::OutOfRangeTx {} => {
            let fraud_proof_verification_response =
                ...
                .ok_or(sp_domains::verification::VerificationError::TxRangeHostFnFailed)?;
            ...
            let is_tx_in_range = match fraud_proof_verification_response {
                ...
                _ => {
                    Err(sp_domains::verification::VerificationError::ReceivedInvalidInfoFromHostFn)
                }
            }?;
            if is_tx_in_range {
                return Err(sp_domains::verification::VerificationError::InvalidProof);
